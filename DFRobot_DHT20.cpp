/*!
 *@file DFRobot_DHT20.cpp
 *@brief Define the basic structure of class DFRobot_DHT20, the implementation of the basic methods
 *@copyright   Copyright (c) 2010 DFRobot Co.Ltd (http://www.dfrobot.com)
 *@licence     The MIT License (MIT)
 *@author [fengli](li.feng@dfrobot.com)
 *@version  V1.0
 *@date  2021-6-24
 *@get from https://www.dfrobot.com
 *@https://github.com/DFRobot/DFRobot_DHT20
*/

#include <DFRobot_DHT20.h>


DFRobot_DHT20::DFRobot_DHT20(TwoWire * pWire,uint8_t address)
  : _pWire(pWire) {
  _address = address;
  _lastRead = 0;
  _lastTemperature = 0.0f;
  _lastHumidity = 0.0f;
}

int DFRobot_DHT20::begin() {
  uint8_t readCMD[3]={0x71};
  uint8_t data;
  delay(100);
  _pWire->begin();
  //check if the IIC communication works 
  writeCommand(readCMD,1);
  
  readData(&data, 1);
  //Serial.println(data);
  if((data | 0x8) == 0){
     return 1;
  }
  if(data == 255) return 1;
  return 0;
}

float DFRobot_DHT20::getTemperature() {
  if(_lastRead + 1000 < millis() || _lastRead == 0){
    startMeasuring();
  }
  return _lastTemperature;
}

float DFRobot_DHT20::getHumidity() {
  if(_lastRead + 1000 < millis() || _lastRead == 0){
    startMeasuring();
  }
  return _lastHumidity*100.0;
}

int DFRobot_DHT20::startMeasuring(){
  int ret = -1;
  uint32_t temp, temp1, rawData;
  uint8_t readCMD[3]={0xac,0x33,0x00};
  uint8_t data[6] = {0};
  int retries = 10;
  // when the returned data is wrong, request to get data again until the data is correct. 
  writeCommand(readCMD, 3);
  while(retries--) {
    delay(80);
    readData(data,6);
    if((data[0] >> 7) == 0){
      //  DBG("bus not busy");
       break;
    }
  }
  // convert humidity
  temp = data[1] & 0xff;
  temp1 = data[2] & 0xff;
  rawData = 0;
  rawData = (temp<<12)+(temp1<<4)+((data[3]&0xf0)>>4);
  // DBG(rawData);
  // DBG(temp<<12);
  _lastHumidity = (float)rawData/0x100000;
  // DBG(humidity)

  // convert temperature
  temp = data[3] & 0xff;
  temp1 = data[4] & 0xff;
  rawData = 0;
  rawData = ((temp&0xf)<<16)+(temp1<<8)+(data[5]);
  // DBG(rawData);
  // DBG((temp&0xf)<<16);
  _lastTemperature = (float)rawData/5242 -50;
  // DBG(temperature)
  
  _lastRead = millis();
  ret = 0;
  return ret;
}


void DFRobot_DHT20::writeCommand(const void *pBuf, size_t size) {
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }

  uint8_t * _pBuf = (uint8_t *)pBuf;
  _pWire->beginTransmission(_address);
  for (uint8_t i = 0; i < size; i++) {
    
    _pWire->write(_pBuf[i]);
    
  }
  _pWire->endTransmission();
}

uint8_t DFRobot_DHT20::readData(void *pBuf, size_t size) {
  delay(10);
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
  uint8_t * _pBuf = (uint8_t *)pBuf;
  //read the data returned by the chip
  _pWire->requestFrom(_address, size);
  for (uint8_t i = 0 ; i < size; i++) {
    _pBuf[i] = _pWire->read();
    DBG(_pBuf[i]);
  }
  return 1;
  
}